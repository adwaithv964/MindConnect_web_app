        ...
        {entries?.length > 0 ? entries.map((entry) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground flex-shrink-0 ml-2">{entry?.date}</span>
            </div>
            <p className="text-sm text-muted-foreground line-clamp-2">{entry?.preview}</p>
            <div className="flex items-center gap-2 mt-2">
              <span className={`px-2 py-1 rounded text-xs font-medium ${entry?.mood === 'positive' ? 'bg-success/10 text-success' :
          ...
          <div className="py-8 text-center text-muted-foreground">
            ...
            <p className="text-xs mt-1">Start writing to track your thoughts.</p>
          </div>
        )}
      </div>

      <div className="flex gap-3">
        <Button variant="outline" fullWidth onClick={onViewAll}>View All</Button>
        <Button variant="default" fullWidth iconName="Plus" onClick={onNewEntry}>New Entry</Button>
      </div>
    </div>
  );
};
